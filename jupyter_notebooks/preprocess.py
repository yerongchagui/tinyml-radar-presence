...
def range_fft(
    x: np.ndarray, window: Optional[Callable[[int], np.ndarray]] = None, remove_mean: bool = True
) -> np.ndarray:
    """Apply a Range FFT.
    ...
    .. tab:: sensor-dsp
        ...
        **Example**
            ...
            Calculate the range fft over a frame with an applied blackman window.
            ...
            ..  code-block:: c
                ...
                float32_t blackman_window[SAMPLES_PER_CHIRP];
                ifx_window_blackman_f32(blackman_window, SAMPLES_PER_CHIRP);

                // input frame from hardware
                float32_t radar_frame[CHIRPS_PER_FRAME * SAMPLES_PER_CHIRP] = ...;

                // complex output range fft
                cfloat32_t range_fft[CHIRPS_PER_FRAME * SAMPLES_PER_CHIRP / 2];

                int32_t result = ifx_range_fft_f32(radar_frame,
                    true, // mean removal
                    range_fft,
                    SAMPLES_PER_CHIRP,
                    CHIRPS_PER_FRAME);
        ...
        **Declaration**
            ...
            .. doxygenfunction:: ifx_range_cfft_f32
                :project: sensor-dsp

            .. doxygenfunction:: ifx_range_fft_f32
                :project: sensor-dsp
    ...
    if x.dtype == np.dtype(np.float32) or x.dtype == np.dtype(np.float64):
        x_range = x_range[..., : n_samples // 2]  # Real data is symmetric

    return x_range


def doppler_fft(x: np.ndarray, window: Optional[Callable[[int], np.ndarray]] = None) -> np.ndarray:
    """Apply a Doppler FFT.
...
def range_doppler_fft(
    x: np.ndarray,
    range_window: Optional[Callable[[int], np.ndarray]] = None,
    doppler_window: Optional[Callable[[int], np.ndarray]] = None,
